
#[macro_use]
extern crate rocket;


use rocket::{Rocket, Build,State,form::Form,http::Status, routes};
use rocket::serde::json::Json;


use serde::{Serialize, Deserialize};
use rocket_db_pools::{Connection, Database};
use toql_mysql_async::MySqlAsync as Toql;
use toql::prelude::{Cache, CacheBuilder, query, fields, paths, ToqlApi, Toql};
use toql_rocket::prelude::{ToqlQuery, Counted};  
use toql_rocket::mysql_async::Result;

mod mysql_async_pool;
use mysql_async_pool::MySqlAsyncPool;


// Here comes our Todo item
// Notice the `Toql` derive -----------------------v
#[derive(Debug, Serialize, Deserialize, PartialEq, Toql)]
pub struct Todo {

    #[serde(default)]
    #[toql(key)]  // <-- Required and must come first
    pub id: u64,

    #[serde(skip_serializing_if = "Option::is_none")]
    pub title: Option<String>,

    #[serde(skip_serializing_if = "Option::is_none", default)]
    pub completed: Option<bool>,
}

//
// Here come the CRUD functions
//
#[delete("/<id>")]
pub async fn delete<'a>(id: u64, mut conn: Connection<ExampleDb>, cache: &State<Cache>) -> Result<Status> 
{
            
    let mut toql = Toql::from(&mut *conn, &*cache);
   
    // Delete an item
    // TodoKey is generated by the Toql derive
    let affected_rows= toql.delete_one(TodoKey::from(id)).await?;  

    Ok(
        if affected_rows == 0 {
            Status::NotFound
        } else {
            Status::NoContent
        }
    )
}

#[put("/<id>", data = "<todo>")]
pub async fn update(
    id: u64,
    mut todo: Json<Todo>,
    cache: &State<Cache>,
    mut conn: Connection<ExampleDb>,
) -> Result<Json<Todo>> {
        
    let mut toql = Toql::from(&mut *conn, &*cache);
  
    // Update all fields from a single entity that have a value
    // fields!(top) means: Check all fields for updates
    let affected_rows = toql.update_one(&mut *todo, fields!(top) ).await?; 

    // Load all fields from updated item
    let q = query!(Todo, "*, id eq ?", id);
    let u = toql.load_one(q).await?;
    Ok(Json(u))
}
git 
#[post("/", data = "<todo>")] 
pub async fn create<'a>(
    mut todo: Json<Todo>,
    cache: &State<Cache>,
    mut conn: Connection<ExampleDb>,
) -> Result<Json<Todo>> {
    
    let mut toql =Toql::from(&mut *conn, &*cache);

    // Insert one item
    // paths!(top) => Ignore all nested joins/merges, not important here
    // This will set the `id` to the generated value
    toql.insert_one(&mut *todo, paths!(top)).await?; 

    // Load all fields from the created item
    let q = query!(Todo, "*, id eq ?", &todo.id);
    let u = toql.load_one(q).await?; 
    Ok(Json(u))
}

#[get("/<id>")]
pub async fn get(
    id: u64,
    cache: &State<Cache>,
    mut conn: Connection<ExampleDb>,
) -> Result<Json<Todo>> {
    
    let mut toql = Toql::from(&mut *conn, &*cache);
   
    // Load all fields from this item 
    let q = query!(Todo, "*, id eq ?", id);
    let u = toql.load_one(q).await?;
    Ok(Json(u))
}

#[get("/?<toql_query..>")]
pub async fn query(
    cache: &State<Cache>,
    mut conn: Connection<ExampleDb>,
    toql_query: ToqlQuery,    // Fetch Toql query and page from URL query parameters
) -> Result<Counted<Json<Vec<Todo>>>> {
    
    let mut toql = Toql::from(&mut *conn, &*cache);

    // Parse query
    let (query, page) = toql_query.parse::<Todo>()?; 

    // Run query
    let r = toql.load_page(query, page).await?;

    Ok(Counted(Json(r.0), r.1))
}

// The database connection
#[derive(Database)]
#[database("example_db")]
pub struct ExampleDb(MySqlAsyncPool);


// Main to startup the server
//fn main() {
#[launch]
pub async fn rocket() -> Rocket<Build>{
    println!("\n");
    println!("Full CRUD example with Rocket, Toql and MySQL");
    println!("=============================================\n");
    println!("This example assumes that you have a MySQL server");
    println!("running with a database `example_db`");
    println!("Run the following SQL to create the table `Todo`");
    println!("CREATE TABLE `Todo` (`id` int(11) NOT NULL AUTO_INCREMENT,`title` varchar(200) NOT NULL, `completed` tinyint(1) PRIMARY KEY (id))");
    println!("");
    println!("Start the server with:");
    println!("ROCKET_DATABASES='{{example_db={{url=mysql://USER:PASS@localhost:3306/example_db}}}}' cargo run");
    println!("");
    println!("Create a todo with `curl localhost:8000/todo -X POST -d '{{\"title\":\"Water plants\"}}'`");
    println!("Update a todo with `curl localhost:8000/todo/ID -X PUT -d '{{\"completed\":\"true\"}}'`");
    println!("Get a single todo with `curl localhost:8000/todo/ID`");
    println!("Get all todos with `curl localhost:8000/todo`");
    println!("Get only completed todos in descending order `curl localhost:8000/todo?query=-id,completed+eq+1`");
    println!("Delete a todo with `curl -X DELETE localhost:8000/todo/ID`");
    println!("\n");

    // Keeps Toql mapping information
    let cache =  CacheBuilder::new().into_cache();
    
    let rocket = rocket::build()
        .manage(cache)
        .attach(ExampleDb::init())
        .mount("/todo", routes![get, query, create, update, delete]);
        

    rocket
}


/* 
impl<'r> rocket::response::Responder<'r, 'static> for  toql_rocket::mysql_async::ToqlMySqlAsyncErrorWrapper {

    fn respond_to(self, request: &'r rocket::Request<'_>) -> std::result::Result<rocket::Response<'static>, Status> {
        let mut response = rocket::Response::new();
      
            Ok(response)
            
        

    }
}
 */